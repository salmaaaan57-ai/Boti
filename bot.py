...
bot = Bot(8787127714:AAEOsu05CBrLstCCs0OQWVMoA3f5TuVQ5CI)
dp = Dispatcher(storage=MemoryStorage())

async def init_db():
    async with aiosqlite.connect('marriage_db.db') as db:
        await db.execute('''CREATE TABLE IF NOT EXISTS users (
            user_id INTEGER PRIMARY KEY, username TEXT, name TEXT, contact_info TEXT, gender TEXT, age TEXT, 
        ...
        try:
            ...
            await db.execute("ALTER TABLE users ADD COLUMN contact_info TEXT")
...
async def process_name(message: Message, state: FSMContext):
    ...
    await message.answer("كيف يمكن للإدارة التواصل معك عند إيجاد طلب مناسب؟\n(يرجى كتابة رقم هاتف، معرف تليجرام، أو بريد إلكتروني):")
...
async def process_bio(message: Message, state: FSMContext):
    ...
    # الجلب التلقائي للبيانات التقنية
    user_id = message.from_user.id
    raw_username = message.from_user.username
    auto_username = f"@{raw_username}" if raw_username else "غير متوفر"
    
    async with aiosqlite.connect('marriage_db.db') as db:
        await db.execute('''INSERT INTO users 
            (user_id, username, name, contact_info, gender, age, social_status, kids, education, job, prayer, smoking, 
            hijab, height, weight, skin_color, health, country, state_name, 
            travel_willingness, marriage_type, housing, partner_specs, bio)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(user_id) DO UPDATE SET 
            username=excluded.username, name=excluded.name, contact_info=excluded.contact_info, gender=excluded.gender, age=excluded.age, social_status=excluded.social_status,
            ...
            ''', 
            (user_id, auto_username, data.get('name'), data.get('contact_info'), data.get('gender'), data.get('age'), data.get('social_status'), 
    ...
    # صياغة استمارة الإدارة (تحتوي على قاطع لفصل البيانات السرية)
    admin_card = (f"📌 **طلب تسجيل جديد للمراجعة**\n"
                  ...
                  f"نبذة: {data.get('bio')}\n"
                  f"➖➖➖➖➖➖➖➖\n"
                  f"🔒 **بيانات تقنية وتواصل (سرية)**:\n"
                  f"المعرف التلقائي: {auto_username}\n"
                  f"الآي دي: `{user_id}`\n"
                  f"التواصل (يدوي): {data.get('contact_info')}")
...
async def admin_approve(callback: CallbackQuery):
    ...
    # قص الجزء السري من الرسالة قبل النشر
    full_text = callback.message.text
    public_part = full_text.split("➖➖➖➖➖➖➖➖")[0]
    ...
    try:
        ...
        await callback.message.edit_text(f"{full_text}\n\n✅ **تم النشر في القناة.**")
    except Exception as e:
